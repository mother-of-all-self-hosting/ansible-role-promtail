...
This is an [Ansible](https://www.ansible.com/) role which installs [Promtail](https://promtailbudget.org) to run as a [Docker](https://www.docker.com/) container wrapped in a systemd service.

Promtail is a local-first personal finance tool.

See the project's [documentation](https://promtailbudget.org/docs/) to learn what Promtail does and why it might be useful to you.
...
To enable Promtail you need to set the hostname as well. To do so, add the following configuration to your `vars.yml` file. Make sure to replace `example.com` with your own value.
...
**Note**: hosting Promtail under a subpath (by configuring the `promtail_path_prefix` variable) does not seem to be possible due to Promtail's technical limitations.
