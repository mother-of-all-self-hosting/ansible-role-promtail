...
#
# Tags look like `v<Promtail version>-<release>`:
#
# - if defaults/main.yml points at a Promtail version that has never been
#   released, the release counter restarts at 0 (`v3.6.11-0`)
# - otherwise the counter is incremented (`v3.6.11-1`), but only if something
...
version="$(sed -nE 's|^promtail_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Promtail version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `3.6.11`), but the tags
# do (`v3.6.11-0`). Stripping any `v` before prepending one keeps this correct
